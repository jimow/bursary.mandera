    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.allocations.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('other_details') ? 'is-invalid' : '' }}" type="text" name="other_details" id="other_details" value="{{ old('other_details', '') }}">
...
@section('scripts')
<script>
document.getElementById('payment_code').addEventListener('keyup', function() {
    this.value = generatePaymentCode();
});
...
function generatePaymentCode() {
    ...
    return `ALL-${month}-${day}-${minute}/${year}`;
}
