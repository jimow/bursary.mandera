...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.schoolAttendance.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.school-attendances.update", [$schoolAttendance->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('admission_number') ? 'is-invalid' : '' }}" name="admission_number_id" id="admission_number_id" required>
                    @foreach($admission_numbers as $id => $entry)
                        <option value="{{ $id }}" {{ (old('admission_number_id') ? old('admission_number_id') : $schoolAttendance->admission_number->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('year') ? 'is-invalid' : '' }}" name="year_id" id="year_id" required>
                    @foreach($years as $id => $entry)
                        <option value="{{ $id }}" {{ (old('year_id') ? old('year_id') : $schoolAttendance->year->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('year'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('school_term') ? 'is-invalid' : '' }}" name="school_term" id="school_term">
                    ...
                    @foreach(App\Models\SchoolAttendance::SCHOOL_TERM_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('school_term', $schoolAttendance->school_term) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('school_name') ? 'is-invalid' : '' }}" name="school_name_id" id="school_name_id">
                    @foreach($school_names as $id => $entry)
                        <option value="{{ $id }}" {{ (old('school_name_id') ? old('school_name_id') : $schoolAttendance->school_name->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
